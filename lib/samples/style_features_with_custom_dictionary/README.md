...
![Image of style features with custom dictionary](style_features_with_custom_dictionary.png)
...
Use the segmented buttons to toggle between the dictionary symbols from the web style and style file. Pan and zoom around the map to see the symbology from the chosen dictionary symbol style. The web style and style file are slightly different to each other to give a visual indication of the switch between the two.

## How it works

1. Add the restaurant feature layer to the map's `operationalLayers` list.
2. Create a `DictionaryRenderer` using `DictionarySymbolStyle.withFileUri(styleFileUri)` with a local stylx file.
3. Create a `DictionaryRenderer` using `DictionarySymbolStyle.withPortalItem(portalItem)` with a `PortalItem`, referring to a `Portal` and the item ID of the web style.
4. Based on the style selected, load the corresponding `DictionaryRenderer` by setting the feature layer's `renderer` property.

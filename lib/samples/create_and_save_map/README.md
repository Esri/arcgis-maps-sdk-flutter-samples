...
![Image of create and save map](create_and_save_map.png)
...
3. Access the `PortalUserContent` with `portal.user!.fetchContent()`, to get the user's list of portal folders with `content.folders`.
4. Create an `ArcGISMap` with a `BasemapStyle` and a few operational layers.
5. Call `map.saveAs()` to save a new `ArcGISMap` with the specified title, tags, and folder to the portal.

...
![image of display alternate symbols at different scales](display_alternate_symbols_at_different_scales.png)
...
1. Create a `FeatureLayer` using a service feature table and add it to the map's list of operational layers.
...
4. Create a `UniqueValue` using the red triangle from step 3 and the list of alternate symbols from step 2.
5. Create a `UniqueValueRenderer` and add the unique value from step 4 to it.
6. Create a purple diamond symbol and convert it to a multilayer symbol to be used as the default symbol.
...
9. Apply the unique value renderer to the feature layer's renderer.

## Relevant API

* ArcGISMapViewController.onViewpointChanged
* FeatureLayer

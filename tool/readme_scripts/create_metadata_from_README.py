...
def create_one_metadata(path: str, category: str):
    """
    ...
    """
    creator = MetadataCreator(path, category)
    try:
        ...
        return
    creator.flush_to_json_file(os.path.join(path, 'README.metadata.json'))


def main():
    msg = 'Create metadata from README. Run it against the folder of a ' \
          'single sample. E.g. /samples/display_map'
    parser = argparse.ArgumentParser(description=msg)
    parser.add_argument('-s', '--sample', help='path to a single sample')
    parser.add_argument('-c', '--category', help='the category name')
    args = parser.parse_args()

    if args.sample:
        create_one_metadata(args.sample, args.category)
    else:
        print('Please provide a path to a sample directory.')

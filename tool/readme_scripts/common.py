...
class Metadata:
    ...
    def __init__(self, folder_path: str, category: str):
        """
        ...
        self.category = category    # Populate from constructor.
    ...
    def get_source_code_paths(self) -> List[str]:
        ...
        :return: A list of dart source code filenames.
        """
        results = []
        for file in os.listdir(self.folder_path):
            if os.path.splitext(file)[1] in ['.dart']:
                results.append(file)
        if not results:
            raise Exception('Unable to get dart source code paths.')
    ...
    def populate_from_readme(self) -> None:
        ...
        try:
            ...
            if self.title.lower().replace(" ", "_") != self.folder_name:
                raise Exception(f'Folder name incorrect: "{self.folder_name}". The folder name should match the sample title.')
            ...
            print(f'Error parsing README - {self.readme_path}.')
            raise err
...
class Readme:
    ...
    def __init__(self, folder_path: str):
        """
        The standard format of README.md. Read more at:
